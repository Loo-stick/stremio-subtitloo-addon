...
class OpenSubtitlesClient {
    ...
    async searchSubtitles({ imdbId, type, season, episode }) {
        ...
        try {
            ...
            console.log(`[OpenSubtitles] ${response.data.length} sous-titre(s) trouvé(s), limite à ${MAX_SUBTITLES}`);

            return response.data.slice(0, MAX_SUBTITLES);
        } catch (error) {
            if (error instanceof RateLimitError) {
                console.error('[OpenSubtitles] Recherche abandonnée (rate limit)');
            } else {
                console.error(`[OpenSubtitles] Erreur recherche:`, error.message);
            }
            return [];
        }
    }
     ...
     */
    formatForStremio(subtitles, addonUrl) {
        const formatted = [];
        ...
        for (const sub of subtitles) {
            try {
                ...
                formatted.push({
                    ...
                    SubDisplayTitle: `[OS] [${downloadCount}↓] ${releaseInfo}`,
