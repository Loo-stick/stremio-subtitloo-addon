    ...
    formatForStremio(subtitles) {
        ...
        for (const sub of subtitles) {
            try {
                ...
                const author = sub.author || 'Anonymous';
                const isHearingImpaired = sub.hi ? ' [HI]' : '';

                // Lang personnalisé pour affichage enrichi
                const langDisplay = `fr ${releaseName}${isHearingImpaired}`;

                formatted.push({
                    id: `subdl-${sub.sd_id || Date.now()}`,
                    url: downloadUrl,
                    lang: langDisplay,
                    SubEncoding: 'UTF-8',
                    SubFormat: 'srt',
                    SubFileName: `${releaseName}.srt`,
                    // Informations affichées dans Stremio
                    SubDisplayTitle: `[SubDL] ${releaseName}${isHearingImpaired}`,
                    SubAuthorComment: `Par ${author}`,
