...
class SubtitleChecker {
     ...
     * @param {boolean} [options.enableYify] - Activer YIFY (défaut: true)
     */
    constructor(options = {}) {
        this.osApiKey = options.osApiKey;
        this.osUserAgent = options.osUserAgent || 'stremio-subtitles-fr v1.4';
        this.subdlApiKey = options.subdlApiKey;
        this.enableYify = options.enableYify !== false;
        ...
        if (this.enableYify) sources.push('YIFY');
     ...
     * @returns {Promise<Object>} { available, count, sources: { os, subdl, yify } }
     */
    async checkAll(imdbId, type) {
        ...
        // YIFY (films uniquement)
        if (this.enableYify && type === 'movie') {
            promises.push(this._checkYIFY(imdbId));
            sourceNames.push('yify');
        } else {
            promises.push(Promise.resolve(null));
            sourceNames.push('yify');
        }
     ...
     * @private
     */
    async _checkYIFY(imdbId) {
        try {
            const results = await yifysubtitles.search({ imdbid: imdbId });

            if (!results) return { available: false, count: 0 };

            const frenchSubs = results.fr || results.french || [];
            const count = frenchSubs.length;

            return { available: count > 0, count };
        } catch (error) {
            console.error(`[SubtitleChecker] YIFY erreur: ${error.message}`);
            return null;
        }
    }
}
